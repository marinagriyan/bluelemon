package com.bluelemon.bluelemon.Models;

import com.intrusoft.sectionedrecyclerview.Section;

import java.io.Serializable;
import java.util.List;

public class AccidentBody{
    private Object injuredParty;
    private String accidentInvestigatorUserName;
    private String categoryName;
    private String departmentName;
    private String severityName;
    private String accidentTypeName;
    private String reference;
    private String accidentInvestigationStageName;
    private Integer accidentID;
    private Integer accidentCategoryID;
    private String accidentDate;
    private String modifiedDate;
    private String accidentSiteID;
    private String accidentVenueLocation;
    private Integer accidentDepartmentId;
    private String accidentFirstAiderName;
    private Boolean emergencyServicesCalled;
    private String emergencyServiceComment;
    private Integer accidentSeverityID;
    private String description;
    private String immediateCause;
    private String causeComments;
    private Integer accidentTypeId;
    private String accidentTypeComments;
    private String partOfBody;
    private String poBComment;
    private String sideOfBody;
    private String immediateAction;
    private Boolean isRiddorIncident;
    private String reportedDate;
    private Integer accidentInvestigationStageID;
    private String accidentInvestigatorUserID;
    private Integer accidentInvestigationLevel;
    private String accidentInvestigationComment;
    private Object humanFactorID;
    private String humanFactorComments;
    private String nearMiss;
    private Object environmentalFactorID;
    private String environmentalComments;
    private Object equipmentFactorID;
    private String equipmentComments;
    private String significantFindings;
    private String addedByUserID;
    private String reportedByFirstName;
    private String reportedBySurname;
    private String reportedByAddress1;
    private String reportedByAddress2;
    private String reportedByAddress3;
    private String reportedByAddress4;
    private String reportedByPostCode;
    private String reportedByLandline;
    private String reportedByMobile;
    private String reportedByEmail;
    private String reportedByOccupation;
    private String reportedByJobTitle;
    private Object organisationFactorID;
    private String organisationFactorComments;
    private Object signedOffByUserID;
    private String additionalPersons;
    private String witnesses;
    private String firstAidComments;
    private String accidentCommentary;
    private String workingConditions;
    private String riskAssessments;
    private String similarRisks;
    private String safeWorkingProcedures;
    private String competentPersons;
    private String accidentAreaObservations;
    private String rootCauses;
    private String chartValue;
    private Object syncDateTime;
    private Object syncUserID;
    private String syncDeviceInfo;
    private Object syncRowGuid;
    private Boolean syncIsModified;

    public Object getInjuredParty() {
        return injuredParty;
    }

    public String getAccidentInvestigatorUserName() {
        return accidentInvestigatorUserName;
    }

    public String getCategoryName() {
        return categoryName;
    }

    public String getDepartmentName() {
        return departmentName;
    }

    public String getSeverityName() {
        return severityName;
    }

    public String getAccidentTypeName() {
        return accidentTypeName;
    }

    public String getReference() {
        return reference;
    }

    public String getAccidentInvestigationStageName() {
        return accidentInvestigationStageName;
    }

    public Integer getAccidentID() {
        return accidentID;
    }

    public Integer getAccidentCategoryID() {
        return accidentCategoryID;
    }

    public String getAccidentDate() {
        return accidentDate;
    }

    public String getModifiedDate() {
        return modifiedDate;
    }

    public String getAccidentSiteID() {
        return accidentSiteID;
    }

    public String getAccidentVenueLocation() {
        return accidentVenueLocation;
    }

    public Integer getAccidentDepartmentId() {
        return accidentDepartmentId;
    }

    public String getAccidentFirstAiderName() {
        return accidentFirstAiderName;
    }

    public Boolean getEmergencyServicesCalled() {
        return emergencyServicesCalled;
    }

    public String getEmergencyServiceComment() {
        return emergencyServiceComment;
    }

    public Integer getAccidentSeverityID() {
        return accidentSeverityID;
    }

    public String getDescription() {
        return description;
    }

    public String getImmediateCause() {
        return immediateCause;
    }

    public String getCauseComments() {
        return causeComments;
    }

    public Integer getAccidentTypeId() {
        return accidentTypeId;
    }

    public String getAccidentTypeComments() {
        return accidentTypeComments;
    }

    public String getPartOfBody() {
        return partOfBody;
    }

    public String getPoBComment() {
        return poBComment;
    }

    public String getSideOfBody() {
        return sideOfBody;
    }

    public String getImmediateAction() {
        return immediateAction;
    }

    public Boolean getRiddorIncident() {
        return isRiddorIncident;
    }

    public String getReportedDate() {
        return reportedDate;
    }

    public Integer getAccidentInvestigationStageID() {
        return accidentInvestigationStageID;
    }

    public String getAccidentInvestigatorUserID() {
        return accidentInvestigatorUserID;
    }

    public Integer getAccidentInvestigationLevel() {
        return accidentInvestigationLevel;
    }

    public String getAccidentInvestigationComment() {
        return accidentInvestigationComment;
    }

    public Object getHumanFactorID() {
        return humanFactorID;
    }

    public String getHumanFactorComments() {
        return humanFactorComments;
    }

    public String getNearMiss() {
        return nearMiss;
    }

    public Object getEnvironmentalFactorID() {
        return environmentalFactorID;
    }

    public String getEnvironmentalComments() {
        return environmentalComments;
    }

    public Object getEquipmentFactorID() {
        return equipmentFactorID;
    }

    public String getEquipmentComments() {
        return equipmentComments;
    }

    public String getSignificantFindings() {
        return significantFindings;
    }

    public String getAddedByUserID() {
        return addedByUserID;
    }

    public String getReportedByFirstName() {
        return reportedByFirstName;
    }

    public String getReportedBySurname() {
        return reportedBySurname;
    }

    public String getReportedByAddress1() {
        return reportedByAddress1;
    }

    public String getReportedByAddress2() {
        return reportedByAddress2;
    }

    public String getReportedByAddress3() {
        return reportedByAddress3;
    }

    public String getReportedByAddress4() {
        return reportedByAddress4;
    }

    public String getReportedByPostCode() {
        return reportedByPostCode;
    }

    public String getReportedByLandline() {
        return reportedByLandline;
    }

    public String getReportedByMobile() {
        return reportedByMobile;
    }

    public String getReportedByEmail() {
        return reportedByEmail;
    }

    public String getReportedByOccupation() {
        return reportedByOccupation;
    }

    public String getReportedByJobTitle() {
        return reportedByJobTitle;
    }

    public Object getOrganisationFactorID() {
        return organisationFactorID;
    }

    public String getOrganisationFactorComments() {
        return organisationFactorComments;
    }

    public Object getSignedOffByUserID() {
        return signedOffByUserID;
    }

    public String getAdditionalPersons() {
        return additionalPersons;
    }

    public String getWitnesses() {
        return witnesses;
    }

    public String getFirstAidComments() {
        return firstAidComments;
    }

    public String getAccidentCommentary() {
        return accidentCommentary;
    }

    public String getWorkingConditions() {
        return workingConditions;
    }

    public String getRiskAssessments() {
        return riskAssessments;
    }

    public String getSimilarRisks() {
        return similarRisks;
    }

    public String getSafeWorkingProcedures() {
        return safeWorkingProcedures;
    }

    public String getCompetentPersons() {
        return competentPersons;
    }

    public String getAccidentAreaObservations() {
        return accidentAreaObservations;
    }

    public String getRootCauses() {
        return rootCauses;
    }

    public String getChartValue() {
        return chartValue;
    }

    public Object getSyncDateTime() {
        return syncDateTime;
    }

    public Object getSyncUserID() {
        return syncUserID;
    }

    public String getSyncDeviceInfo() {
        return syncDeviceInfo;
    }

    public Object getSyncRowGuid() {
        return syncRowGuid;
    }

    public Boolean getSyncIsModified() {
        return syncIsModified;
    }
}
